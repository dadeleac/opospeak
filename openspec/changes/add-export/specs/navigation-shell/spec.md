...
The Ajustes tab SHALL contain only what does not belong to the practice flow: app information (name, version), a visible privacy statement (local-first, no account, user owns the data), a functional "Exportar mis datos" action that generates the full export package defined in `export-package`, and a placeholder row for iCloud status that communicates the feature arrives in an upcoming version. Ajustes SHALL NOT become a feature drawer.

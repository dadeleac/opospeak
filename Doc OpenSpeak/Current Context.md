...
- **Attempt deletion** (OpenSpec change `add-attempt-deletion`, 2026-06-12): attempts are deletable — swipe on the Ficha's Historial rows and "Eliminar intento" in the attempt detail's "···" menu, both behind the same confirmation alert (irreversible audio: the cost dictates the friction, in deliberate contrast to alert-free note deletion). Always through `PracticeRepository` (single deletion path, file never orphans). Accepted consequence: insights recalculate without the deleted practice. Deferred: audio-only deletion keeping the attempt (storage management) — its own future change.
- **Curriculum deletion and syllabus rename** (OpenSpec change `add-curriculum-deletion`, 2026-06-12): data ownership at every level. `PracticeRepository` now covers the whole hierarchy — `delete(topic:)` and `delete(syllabus:)` remove every audio file underneath before deleting the model (a raw `modelContext.delete` would orphan them; both paths repository-tested). UI: swipe "Eliminar" next to "Archivar" in the topic and syllabus lists (archive preserves, delete destroys — both visible, different friction), red destructive rows at the bottom of the edit sheets (HIG pattern), new `EditSyllabusSheet` for renaming. Confirmation alerts state the real scale (N temas / N intentos with recordings). Deleting from an edit sheet also closes the screen that lost its subject.
